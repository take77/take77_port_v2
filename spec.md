...
### 5.1 カラーパレット（2モード制 / 2026-06改訂）

デフォルト（ライトモード）= A2「ネイビー×ピュアホワイト」、ダークモード = B1「ネイビー×セージ」。
すべて `global.css` のセマンティックCSS変数（`--accent`、`--card-bg` 等）で管理し、コンポーネントへの生値直書きは禁止。
モード切替は class 戦略（`html.dark`）で、初期値は `localStorage.theme` > `prefers-color-scheme` の優先順。
実装指示の詳細は `docs/color-redesign-instructions.md` を参照。

**ライトモード（デフォルト）— A2「ネイビー×ピュアホワイト」**

```
背景グラデーション:    linear-gradient(135deg, #fbfbfd 0%, #f3f5f8 55%, #edf0f4 100%)
テキスト（見出し）:    #1c2138
テキスト（本文）:      #2b2f45
テキスト（サブ）:      rgba(43, 47, 69, 0.55)
テキスト（アクセント）: #353a5c
アクセントカラー:      #353a5c
アクセント（ホバー）:   #2a2e4a
アクセント（淡）:      rgba(53, 58, 92, 0.10)   — バッジ/タグ背景用
アクセント上の文字:    #ffffff
カード背景:           rgba(255, 255, 255, 0.78)
カードボーダー:        rgba(48, 52, 77, 0.10)
カードシャドウ:        0 10px 30px rgba(48, 52, 77, 0.08)
ナビ背景（スクロール時）: rgba(251, 251, 253, 0.82)
入力欄背景:           rgba(255, 255, 255, 0.92)
入力欄ボーダー:        rgba(48, 52, 77, 0.16)
グリッドパターン線:    rgba(48, 52, 77, 0.06)
デュオトーンベース:    #e8eaef
カード下端フェード:    rgba(251, 251, 253, 0.92)
アクセントブロブ:      rgba(74, 81, 112, 0.16)    — ネイビー（プライマリ）
                      rgba(140, 150, 121, 0.16)  — セージ
                      rgba(201, 191, 169, 0.22)  — ベージュ
                      rgba(53, 58, 92, 0.06)     — ネイビー淡（深度用/アンビエント）
```

**ダークモード — B1「ネイビー×セージ」**

```
背景グラデーション:    linear-gradient(135deg, #2a2e42 0%, #181b28 55%, #20242f 100%)
テキスト（見出し）:    #ffffff
テキスト（本文）:      #f0f0f5
...
アクセントカラー:      #a3b18a
アクセント（ホバー）:   #b6c3a0
アクセント（淡）:      rgba(163, 177, 138, 0.16)
アクセント上の文字:    #1f2335
カード背景:           rgba(255, 255, 255, 0.06)
カードボーダー:        rgba(255, 255, 255, 0.10)
カードシャドウ:        0 8px 32px rgba(0, 0, 0, 0.35),
                      inset 0 1px 0 rgba(255, 255, 255, 0.08)
ナビ背景（スクロール時）: rgba(24, 27, 40, 0.72)
入力欄背景:           rgba(255, 255, 255, 0.06)
入力欄ボーダー:        rgba(255, 255, 255, 0.14)
グリッドパターン線:    rgba(255, 255, 255, 0.05)
デュオトーンベース:    #23263a
カード下端フェード:    rgba(24, 27, 40, 0.92)
アクセントブロブ:      rgba(140, 150, 121, 0.26)  — セージ（プライマリ）
                      rgba(201, 191, 169, 0.14)  — ベージュ
                      rgba(74, 81, 112, 0.30)    — ネイビーブルー
                      rgba(163, 177, 138, 0.10)  — セージ淡（深度用/アンビエント）
```

**コントラスト**: ライト アクセント #353a5c vs 白文字 = 10.99:1、本文 vs 背景 = 12.59:1 / ダーク アクセント vs ボタン文字 = 6.82:1、本文 vs 背景 = 15.08:1。いずれも WCAG AA（4.5:1）クリア。
...
- ダーク: `opacity: 0.035` + `mix-blend-mode: overlay` / ライト: `opacity: 0.025` + `mix-blend-mode: multiply`（overlayは明背景でほぼ視認できないため）
...
| ブロブ | サイズ | 色 | 動き | 周期 |
|-------|-------|-----|------|------|
| Primary | 550px | `var(--blob-1)` | 広い楕円軌道 + スケールパルス | 28s |
| Blob 2 | 480px | `var(--blob-2)` | カウンター周回、非対称イージング | 24s |
| Blob 3 | 420px | `var(--blob-3)` | タイトな周回 | 19s |
| Ambient | 350px | `var(--blob-4)` | 超低速ドリフト + 不透明度パルス | 35s |

モード別の色は 5.1 を参照。すべて `cubic-bezier` による緩急のあるイージングを使用。`filter: blur(80〜100px)`。

**③ グリッドパターン（ヒーローセクションのみ）**
- 80px間隔の格子線、線色は `var(--grid-line)`（モード別の値は5.1参照）
...
モード別の実値：
- ダーク: 背景 `rgba(255,255,255,0.06)` / ボーダー `rgba(255,255,255,0.10)` / シャドウ `0 8px 32px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.08)`
- ライト: 背景 `rgba(255,255,255,0.78)` / ボーダー `rgba(48,52,77,0.10)` / シャドウ `0 10px 30px rgba(48,52,77,0.08)`（**insetハイライトなし**）

**ホバー状態：**
```css
.glass:hover {
  border-color: var(--card-border-hover); /* ダーク rgba(255,255,255,0.18) / ライト rgba(48,52,77,0.22) */
...
| ダークパープル背景 | **2モードのネイビー基調に変更（2026-06改訂）**: ライト=ネイビー×ピュアホワイト / ダーク=ネイビー×セージ（5.1参照） |
...
| ファイル | 内容 | ステータス |
|---------|------|----------|
| `prototype-v5.jsx` | デスクトップ版プロトタイプ（全ページ遷移、ビジュアル確定）※カラーは `docs/color-redesign-instructions.md` が優先（レイアウトの参照元としては有効） | ✅ 承認済み |
...
| `docs/color-redesign-instructions.md` | カラーリデザイン指示書（パープル→ネイビー×セージ、2モード化） | ✅ 承認済み（2026-06） |
| `palette-variations.jsx` | カラーパレット検討バリエーション（ライト=A2 / ダーク=B1 採用） | ✅ 承認済み（2026-06） |
